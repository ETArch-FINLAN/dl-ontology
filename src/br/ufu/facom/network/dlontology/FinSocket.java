...
public class FinSocket {
	...
	//Parser
	private static DLParser parser = new SimpleParser();
	private static ServiceMessageParser serviceParser = new SerializedParser();
	...
	public boolean register(String title){
		ServiceRequest request = new ServiceRequest(Service.ENTITY_REGISTER);
		request.putProperty("title",title);
		
	
		if(write(Constants.dtsWorkspace,serviceParser.unmarshall(request))){
			
			while(true){
				Message inMessage = read();
			
				ServiceMessage serviceMessage = serviceParser.marshall(inMessage.getPayload());
				if(serviceMessage != null && serviceMessage instanceof ServiceResponse){
					if(((ServiceResponse)serviceMessage).isSucess()){
						this.title = title;
						...
						debug("Register fail: "+((ServiceResponse)serviceMessage).getMessage());
					}
					return false;
				}else{
	...
	public boolean unregister(){
		ServiceRequest request = new ServiceRequest(Service.ENTITY_UNREGISTER);
		request.putProperty("title",title);
			...
			while(true){
				...
				if(serviceMessage != null && serviceMessage instanceof ServiceResponse){
					if(((ServiceResponse)serviceMessage).isSucess()){
						...
						debug("Unregister fail: "+((ServiceResponse)serviceMessage).getMessage());
					}
	...
	public boolean joinWorkspace(String workspace){
		if(!this.workspaces.containsKey(workspace)){
			ServiceRequest request = new ServiceRequest(Service.WORKSPACE_ATTACH);
			...
			if(write(Constants.dtsWorkspace,serviceParser.unmarshall(request))){
				while(true){
					...
					if(serviceMessage != null && serviceMessage instanceof ServiceResponse){
						if(((ServiceResponse)serviceMessage).isSucess()){
							workspaces.put(workspace, Integer.parseInt(((ServiceResponse)serviceMessage).getReturnValue().toString()));
							return true;
						}
	...
	public boolean disjoinWorkspace(String workspace){
		if(this.workspaces.containsKey(workspace)){
			ServiceRequest request = new ServiceRequest(Service.WORKSPACE_DETACH);
			request.putProperty("title",workspace);
	...
	public boolean createWorkspace(String workspace){
		ServiceRequest request = new ServiceRequest(Service.WORKSPACE_CREATE);
		request.putProperty("title",workspace);
	...
	public boolean deleteWorkspace(String workspace){
		ServiceRequest request = new ServiceRequest(Service.WORKSPACE_DELETE);
		request.putProperty("title",workspace);
