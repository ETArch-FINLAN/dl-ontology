...
public class DlopSimpleParser implements DlopParser{
	//PatternsMessage
	...
	public byte[] parse(DlopMessage message){
	...
	public DlopMessage parseMessage(byte[] data){
		...
		return new DlopMessage(source,destin,payload,fragmented,sequence);
	}
	...
	@Override
	public List<DlopMessage> fragmentMessage(String title, int vlan, String destin, byte[] msg) {
		int overhead=202;
		...
		List<DlopMessage> list = new ArrayList<DlopMessage>();
		int offset=0;
		int size = msg.length;
		
		int pos = 0;
		while((overhead+fields+(size-offset))+(""+pos).length() > DlopMessage.MAX_FRAME_SIZE){
			int parcial = DlopMessage.MAX_FRAME_SIZE - (overhead+fields) + 1 - ((""+pos).length());
			
			list.add(new DlopMessage(title, destin, vlan, Arrays.copyOfRange(msg, offset, offset+parcial), true,sequenceActual+"."+(pos++)));
		...
		if(pos>0)
			...
			list.add(new DlopMessage(title, destin, vlan, Arrays.copyOfRange(msg, offset, size),false,seqStr));
